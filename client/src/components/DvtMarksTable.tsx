...
type ColorClass = 'bg-gray-100 text-gray-400' | 'bg-red-100 text-red-800' | 'bg-yellow-100 text-yellow-800' | 'bg-blue-100 text-blue-800' | 'bg-green-100 text-green-800';
...
const DvtMarksTable: React.FC = () => {
  ...
  const fetchDvtData = async () => {
    ...
    try {
      ...
      const sortedData = data.data.sort((a: DayData, b: DayData) => new Date(b.date).getTime() - new Date(a.date).getTime());
  ...
  const getTotalForDay = (dayData: DayData): number => {
    return classes.reduce((total, classNum) => total + (dayData.classes[classNum] || 0), 0);
  };

  const getTotalForClass = (classNum: number): number => {
    return tableData.reduce((total, day) => total + (day.classes[classNum] || 0), 0);
  };
        ...
        <table className="w-full border-collapse border border-gray-300 rounded-lg overflow-hidden">
          ...
          <tbody>
            {tableData.map((dayData: DayData, index: number) => (
              <tr key={dayData.date} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                ...
                {classes.map((classNum: number) => {
                  ...
                  return (
                    ...
                    >
                      <span className={`inline-block px-2 py-1 rounded text-sm font-medium ${getColorClass(count)}`}>
                        {count}
                      </span>
                    </td>
                  );
